...
class GMMDataset(torch.utils.data.Dataset):
    ...
    def gaussian_mixture(self):
        """
        one-dimensional Gaussian mixture generator
        ...
        return:
        y of shape (num_samples), components
        """
        output = torch.zeros_like(self.X)
        components = []
        for index in range(self.num_components): 
            dist = stats.norm(self.means[index], self.stds[index])
            component = torch.FloatTensor(dist.pdf(x=self.X)) * self.mixture_coefficient[index]
